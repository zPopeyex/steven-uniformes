  ...
  const handleAgregarVenta = async (itemsCarrito) => {
    try {
      ...
      if (itemsInvalidos.length > 0) {
        throw new Error(`${itemsInvalidos.length} producto(s) no tienen todos los campos requeridos o valores inválidos`);
      }
    ...
    <div style={{ padding: 20 }}>
      ...
      {tabActiva === "ventas" ? (
        <>
          ...
          <VentasTable 
            ventas={ventas} 
            onActualizarEstado={(id, estado) => handleActualizarEstado(id, estado, "ventas")}
          />
        </>
      ) : (
        <EncargosTable 
          encargos={encargos} 
          onActualizarEstado={(id, estado) => handleActualizarEstado(id, estado, "encargos")}
        />
      )}
    </div>
  );
